 ...
 *
 * Now answer two questions: Note that the width of a character is not 1!
 ...
 */
public class _1011_NumberOfLinesToWriteString {

    /**
     * 遍历每个 char, 计算当前总宽度 + 当前字符宽度, 判断是否 > 100.
     * - 如果是: 行数递加, 将当前总宽度赋值为当前字符宽度
     * - 如果不是: 增加当前总宽度
     */
    public int[] numberOfLines(int[] widths, String S) {

        int lines = 1; // 注意: lines 从 1 开始, 不然会少数一行
        int curWidth = 0;

        for (int i = 0; i < S.length(); i++) {
            int charIndex = S.charAt(i) - 'a';
            curWidth += widths[charIndex];
            if (curWidth > 100) {
                lines++;
                curWidth = widths[charIndex];
            }
