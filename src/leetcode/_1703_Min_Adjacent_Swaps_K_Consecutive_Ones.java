...
public class _1703_Min_Adjacent_Swaps_K_Consecutive_Ones {
    ...
    public int minMoves(int[] nums, int k) {
        ...
        // prefixSum[j]-prefixSum[i]表示从 i+1 位加到 j 位的数组和,  注意是从 i + 1 开始到 j
        ...
        // k=9 -> 4 3 2 1 0 1 2 3 4 -> (1+2+3+4)x2 -> ((1+4)x4/2)x2 -> (1+4)x4 -> (k+1)/2 x (k-1)/2
        //                                           因为是奇数, 所以可以归纳成    -> (k+1)/2 x k/2
        // k=8 ->   3 2 1 0 1 2 3 4 -> (1+2+3)x2+4 -> (1+3)x3/2x2+4 ->(1+3)3+4 -> (1+3)x(1+3) -> k/2 x k/2
        //                                           因为是偶数, 所以可以归纳成    -> (k+1)/2 x k/2
